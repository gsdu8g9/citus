				...
				bool hasMetadata, uint32 nodeState, bool *nodeAlreadyExists)
{
	...
	*nodeAlreadyExists = false;
	...
	InsertNodeRow(nextNodeIdInt, nodeName, nodePort, groupId, nodeRack, hasMetadata, nodeState);
...
GenerateNodeTuple(WorkerNode *workerNode)
{
	...
	values[Anum_pg_dist_node_state - 1] = UInt32GetDatum(workerNode->nodeState);
...
TupleToWorkerNode(TupleDesc tupleDescriptor, HeapTuple heapTuple)
{
	...
	Datum hasMetadata = heap_getattr(heapTuple, Anum_pg_dist_node_hasmetadata,
									 tupleDescriptor, &isNull);
	Datum nodeState = heap_getattr(heapTuple, Anum_pg_dist_node_state,
									 tupleDescriptor, &isNull);
	...
	workerNode->nodeState = DatumGetUInt32(nodeState);
